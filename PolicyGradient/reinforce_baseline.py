...
def policy_gradient(env, alpha, gamma, num_episodes):
    ...
    optimizer1 = optim.Adam(policy_net.parameters(), lr=alpha)
    optimizer2 = optim.Adam(value_net.parameters(), lr=alpha)
    reward_history = []
    ...
    for episode in range(num_episodes):
        ...
        values_tensor = torch.stack(values).squeeze(-1)  # Ensure correct shape
        advantages = torch.tensor(total_rewards, dtype=torch.float32) - values_tensor

        # Update the policy network
        loss1 = -torch.sum(
            torch.stack(log_probs) * advantages
        )  # Detach advantages here
        optimizer1.zero_grad()
        loss1.backward()
        optimizer1.step()

        # Update the value network
        loss2 = torch.sum((advantages.detach()) ** 2)
...
if __name__ == "__main__":
    ...
    torch.save(
        trained_policy_net.state_dict(),
        "PolicyGradient/reinforce_baseline_cartpole.pth",
    )
